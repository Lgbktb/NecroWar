...
class AI(BaseAI):
    ...
    def get_name(self):
        ...
        return "TotallyNotARobot" # REPLACE THIS WITH YOUR TEAM NAME
    ...
    def run_turn(self):
        ...
        # Test Spawn Many Workers
        if(self.player.gold > 1 and self.player.mana > 10):
            if(self.wraithBuilt == False):
                if self.spawnUnitTile.spawn_unit("wraith"):
                        self.units.append(self.player.units[-1])
                        self.wraithBuilt = True
            for x in range(0,4):
                if self.spawnUnitTile.spawn_unit("hound"):
                    self.units.append(self.player.units[-1])
    ...
    def find_path(self, start, goal):
        ...
        while len(fringe) > 0:
            ...
            for neighbor in inspect.get_neighbors():
                ...
                if neighbor and neighbor.id not in came_from:
